...
def test_bg_finishes_immediately(num_bg_tasks):
    ...
    async def async_fn():
        async with ag.run_as_daemons(*[finish_imm() for __ in range(num_bg_tasks)]) as bg_tasks:
            for t in bg_tasks:
                assert t.finished
        for t in bg_tasks:
            assert t.finished
...
def test_bg_finishes_while_fg_is_running(num_bg_tasks):
    ...
    async def async_fn():
        async with ag.run_as_daemons(*[e.wait() for __ in range(num_bg_tasks)]) as bg_tasks:
            for t in bg_tasks:
...
def test_fg_finishes_while_bg_is_running(bg_cancel):
    ...
    async def async_fn(e):
        async with ag.run_as_daemons(bg_fn()) as bg_tasks:
...
def test_fg_finishes_while_bg_is_suspended(num_bg_tasks):
    ...
    async def async_fn():
        async with ag.run_as_daemons(*[ag.sleep_forever() for __ in range(num_bg_tasks)]) as bg_tasks:
            for t in bg_tasks:
                assert t.state is TS.STARTED
        for t in bg_tasks:
