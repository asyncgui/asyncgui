...
def test_closer_task_completes_while_there_are_other_running_tasks(wait, daemon):
    import asyncgui as ag
    TS = ag.TaskState
    ...
    async def async_fn():
        async with ag.open_nursery() as nursery:
            s = nursery.start
            for close_on_finish in (False, True):
                for daemon_ in (False, True):
                    s(ag.sleep_forever(), daemon=daemon_, close_on_finish=close_on_finish)
            s(e.wait(), daemon=daemon, close_on_finish=True)
            if wait:
                await ag.sleep_forever()
