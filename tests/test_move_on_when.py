...
def test_bg_finishes_immediately(fg_sleep):
    ...
    async def async_fn():
        async with ag.move_on_when(finish_imm()) as bg_task:
...
def test_bg_finishes_while_fg_is_running(fg_sleep):
    ...
    async def async_fn():
        async with ag.move_on_when(ag.sleep_forever()) as bg_task:
...
def test_bg_finishes_while_fg_is_suspended():
    ...
    async def async_fn():
        async with ag.move_on_when(e.wait()) as bg_task:
            assert bg_task.state is TS.STARTED
            await ag.sleep_forever()
            pytest.fail()
        assert bg_task.state is TS.FINISHED
...
def test_fg_finishes_while_bg_is_running(bg_sleep):
    ...
    async def async_fn():
        async with ag.move_on_when(bg_fn()) as bg_task:
            assert bg_task.state is TS.STARTED
            await ag.sleep_forever()
            assert bg_task.state is TS.STARTED
