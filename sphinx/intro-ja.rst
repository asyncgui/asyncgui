...
``asyncgui`` 自体はシステムコールに繋がるような機能を持ちません(例えば入出力)。
そのような機能を ``asyncgui`` から利用したければとある作業が要ります(詳しくは :doc:`usage-ja` で解説)。
そしてこの一見すると不便に思える所がこのモジュールの売りです。



そしてそれは :func:`asyncio.sleep` のような基本的な機能すら持てない事も意味します。
何故ならその実現には

* 現在時刻の取得 (:func:`time.time` や :func:`time.perf_counter`)
* スレッドの一時停止 (:func:`time.sleep`)

といったシステムコールに繋がるであろう操作を要するからです。(加えてメインループも要ります)。
